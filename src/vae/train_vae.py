...
class Beta_Plane_ML:
    def __init__(self, feat_dim=256, latent_dim=256, seq_len=1, ens_size=4, epochs=1,
        ...
        # Loss function
        self.KL  = lambda mu, sigma: -0.5 * torch.sum(1 + sigma - mu.pow(2) - sigma.exp())/mu.size(0)
...
if __name__ == '__main__':
    for RUN_NUM in range(211, 220, 2):
        for LATENT_DIM in [64]:#[32, 128, 24, 96]:
            train()
